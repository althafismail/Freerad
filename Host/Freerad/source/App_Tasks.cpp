...
//	static char tempString[20] = {0};
	...
	for(;;)
	{
		...
		if((clockIn - clockOut) >= 32)	{
			for(i=0; i<32; i++)	{
				if(notificationValue & (1 << i)) {
					if(	GPIO_ReadPinInput(GPIOC, i))
						sprintf(logString, "GPIOC Pin %d Released\r\n", i);
					else
						sprintf(logString, "GPIOC Pin %d Pressed\r\n", i);
					xQueueSend(Q_Printf, logString, 0);
				}
			}
		}
